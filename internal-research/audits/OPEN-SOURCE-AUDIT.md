...
HuntMCP is in noticeably better shape than the median security-tooling open-source repo, *as source code*. No real secrets or credentials were found anywhere in the tracked tree or in the full git history. There is no exploitable CI trust boundary (no `pull_request_target`, no secrets referenced in workflows, no shell-injection vectors). Scope enforcement for the actual offensive tooling is implemented as a real technical control (a `PreToolUse` hook plus per-tool budget/audit chokepoints), not just documentation, and the agent permission model is consistently least-privilege — most notably, the report-generation agent has no submission capability anywhere in its tool graph or the underlying HackerOne MCP server, which is a genuine, verified enforcement of "human review before submit," not just a stated policy.

**However, this audit also caught a live, active instance of exactly the risk category the codebase is designed to prevent.** While finalizing this report, the primary working tree (`/home/ankit/HuntMCP`, branch `main`) was found to contain uncommitted, untracked operational files with real, live bug-bounty engagement data — a script naming a real target, real account/org IDs, and real credential-file references — sitting directly in `scripts/`, a directory this repository's own `CLAUDE.md` documents as pushed to the public remote. This is not a hypothetical architecture gap; it is live target data one `git add -A && git commit` away from public exposure. See the new P0 finding immediately below. It was left untouched per instruction — only reported.

The remaining issues are release-hygiene and documentation-freshness problems rather than active leaks: a stale `ROADMAP.md` that undersells what's actually built, three documents that disagree on the MCP server count, a per-target SQLite state file (`data/watch.db`) that was tracked in git despite being intended as gitignored local state (historically holding only test-fixture data, though see the P0 note — it was found actively growing during this audit), a docker-compose default that silently enables a hardcoded dev JWT secret if an operator forgets to set env vars, and the absence of a `SECURITY.md`/`CONTRIBUTING.md`/issue templates that a security-tooling project should have.

**Overall: NOT READY — one live, real engagement-data-exposure risk must be resolved by a human before any push to a public remote.** Once that specific risk (below) is cleared, the rest of the findings reduce this to READY WITH FIXES. Work through the immediate-action list below, starting with the P0.
...
**One P0 was found, discovered live during this audit rather than by the static sub-audits (which ran against a clean branch checkout, not the primary repo's actual working tree).** Two further items are elevated to P1 on documentation-integrity and contributor-safety grounds — neither of those two is an active security exposure, but both materially affect whether the project is ready for outside contributors.

### [P0 — Privacy / Engagement Data Separation] Live bug-bounty engagement data found uncommitted in a path that gets pushed publicly

- **Finding:** The primary working tree at `/home/ankit/HuntMCP` (branch `main`, not any audit branch) contains an uncommitted, untracked file `scripts/remind-coderpad-retest.sh` containing: a real target name (CoderPad, coderpad.io), real account/org identifiers (a specific user ID and org ID), a real disclosure-program contact address for that program, references to real saved session/credential files (`jar_b.txt`, `keys.env` under `data/engagements/coderpad-io/`), and a step-by-step plan to re-test a specific submitted finding after a trial expires. Also present in the same untracked/modified state: `data/watch.db` (a per-target SQLite state file, actively grown from 24KB to 104KB during the period this audit ran — see the related P2 below, now demonstrated live rather than theoretical), a modified `mcp-servers/scope_guard.py`, a new `data/writeups/gtm-env-credential-liveness-oracle.md` (this one appears to be a generic technique writeup, not target-specific), and a stray untracked file literally named `-`.
- **Evidence:** `git status --short` in `/home/ankit/HuntMCP` at audit time showed all of the above as uncommitted (`M`/`??`). Full paths and identifiers are deliberately not repeated verbatim here beyond what's needed to locate the file — see the live file itself for specifics, and redact before any further handling.
- **Risk:** `scripts/` is explicitly documented in this repository's own `CLAUDE.md` as a directory that gets pushed to the public remote. If this file (or `data/watch.db`, if it ever holds real per-target data rather than test fixtures) is committed via a broad `git add -A`/`git add .` — exactly the pattern this project's own git-safety guidance warns against — real, live, unresolved bug-bounty engagement details would be pushed to a public, open-source repository. This is categorically the most severe class of finding this audit was tasked to look for, and it was caught only because it happened to be sitting in the working tree at audit time — it would not have been visible to a clean-checkout static audit.
- **Exposure:** Not yet exposed publicly (confirmed uncommitted, `main` has not been pushed with these files per `git status`). This is a near-miss, not a confirmed leak — but a near-miss with the fuse already lit, since the file lives in a push-path.
- **Recommended remediation:** A human must decide the disposition of `scripts/remind-coderpad-retest.sh` (and confirm `data/watch.db`'s current real content) *before* any commit/push touching these paths. Options include: relocate to a gitignored location outside the tracked `scripts/` directory, add a specific `.gitignore` entry if this class of per-engagement reminder script recurs, or delete once no longer needed. This audit deliberately took no action on these files — per instruction, they were left untouched and are only reported here.
- **Validation:** After remediation, re-run `git status --short` in `/home/ankit/HuntMCP` and confirm no target-identifying files remain uncommitted in a pushed path, and `git log --all -- scripts/remind-coderpad-retest.sh` to confirm it was never previously committed either.
...
**NOT READY** (as of the moment this audit ran), pending human resolution of the P0 live-engagement-data finding above.

Justification: the codebase itself — architecture, permission model, secret hygiene, CI trust boundary — is genuinely strong, and taken alone would rate READY WITH FIXES. But this audit caught real, live, uncommitted bug-bounty engagement data (a real target, real account IDs, real credential-file references) sitting in `scripts/`, a path this repo pushes publicly. That must be resolved by a human — not by this audit, which was instructed to observe only — before any commit or push touches those paths. Once resolved, the remaining findings are release-hygiene, documentation-freshness, and governance-scaffolding gaps rather than active security exposures, and the project reduces to **READY WITH FIXES**. None of the remaining items require an architectural change.

---

## Immediate Action List

0. **(P0, blocking)** Resolve `scripts/remind-coderpad-retest.sh` and confirm the current real content of `data/watch.db` in `/home/ankit/HuntMCP` before any commit/push touches `scripts/` or `data/`. This audit took no action on these files by instruction — a human must decide relocate/gitignore/delete.
