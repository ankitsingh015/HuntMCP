...
| [PortSwigger `mcp-server`](https://github.com/PortSwigger/mcp-server) | Official Burp MCP integration — already the design behind the `127.0.0.1:9876` reference in AGENTS.md | Kept as an optional enhancement tier, not a hard requirement. `scripts/connect-burp.sh` registers the bridge as a personal `--scope local` MCP config once installed/started inside Burp — see AGENTS.md's runtime-dependencies section |
| [`h4ckf0r0day/obscura`](https://github.com/h4ckf0r0day/obscura) (Rust, Apache 2.0, 23.5k★) | Lightweight headless browser (30MB RAM/70MB binary vs Chrome's 200MB+/300MB, ~85ms page loads) with a native V8 engine and its own built-in MCP server (32 tools: navigation, snapshot/markdown/extract, click/fill/type, screenshot/PDF, cookies/storage, tabs) — genuine alternative to `browser-mcp`'s Playwright/Chromium for the same JS/DOM-execution-confirmation role. Verified directly against its own docs (not just marketing copy): its "stealth" mode disables GPU rendering entirely and offers profile rotation, but does NOT randomize canvas/audio/battery fingerprints despite some third-party posts claiming full per-session fingerprint randomization — its own docs explicitly state it cannot bypass Cloudflare interactive challenges, active bot managers, CAPTCHAs, or IP-based rate limiting, so it doesn't replace `waf-bypass-mcp`/`playwright-mcp`'s `solve_js_challenge` for that. | ✅ Added as `obscura-mcp`, alongside (not replacing) `browser-mcp` — `scripts/connect-obscura.sh` registers it as a personal `--scope local` MCP config (same reasoning as Burp's own bridge: it's its own compiled binary acting as the MCP server, not a `mcp-servers/*/server.py` FastMCP wrapper, so its calls never pass through `tool_resolver.py`'s shared budget-circuit-breaker/audit-log logic the way every other tool-backed server's do — a real, structural gap shared with Burp's bridge, documented rather than silently accepted; making it *tracked* config instead, as first implemented and then reverted after a code-review pass, would have forced that same gap onto every clone of this repo by default and directly contradicted this row's own reasoning for keeping Burp untracked). Added to `scope_gate_hook.py`'s `TIER2_MCP_SERVERS`, granted to recon-agent/exploit-agent in both harnesses — with the caveat that only `browser_navigate(url)` carries a scope-checkable host; `browser_screenshot`/`browser_evaluate`/etc. act on the already-navigated page and aren't independently re-gated (documented in the hook's own comment and connect-obscura.sh's printed instructions). Untested live in this environment (binary not installed here) — registration/scope-gating verified structurally, not via a real tool call |
...
| **Extension (2026-08-29) — `scripts/curl-rl.sh`: 429/Retry-After auto-retry for raw curl calls** | ✅ Done. `tool_resolver.run_tool()` already gives every Tier-2 MCP-wrapped tool (nuclei/sqlmap/subfinder/etc.) a reactive 429-detect-and-retry-once behavior, but raw `curl`/`wget` Bash calls — now scope-gated, but still the one Tier-2 path with no MCP wrapper at all — never went through it. The natural first instinct (put the retry logic inside `scripts/hooks/scope_gate_hook.py`, a `PreToolUse`/`tool.execute.before` hook) turned out to be architecturally wrong, caught before building it: a hook can only ALLOW or BLOCK a command *before* it runs, it has no way to wrap/replace/retry the actual subprocess execution once the harness runs it. Built as a standalone drop-in wrapper instead — `scripts/curl-rl.sh <same args you'd give curl>` — that always captures response headers to its own temp file (`-D`), inspects them for a `429` status line or a `Retry-After` header, and on a hit backs off 5s and retries exactly once (same one-retry-only rule as `tool_resolver.run_tool()` — a second block means something else is wrong, not a transient limit). Two response-body handling paths, chosen by whether the caller already passed their own `-o`/`-O`/`--output`/`--remote-name(-all)` flag: if they did (matching recon-agent's own documented `curl -o "<dir>/<name>.js" <url>` JS-download pattern), curl writes straight to their file on every attempt as normal, so a retry's write simply (and correctly) overwrites a failed attempt's content; if they didn't (a plain curl-to-stdout call), the body is captured to an internal temp file and only the FINAL attempt's body is `cat`-ed at the end — a real bug caught live in the first version, which let both the failed and retried attempt write directly to stdout, producing a concatenated 429-error-then-real-response blob. `scope_gate_hook.py`'s `TIER2_BASH_TOOLS` and its curl/wget-specific budget+audit block both now recognize `curl-rl.sh` identically to raw `curl`/`wget` (via the existing `_first_word()` path-stripping helper — no new host-extraction logic needed, since the existing URL/hostname regex scan already works on the whole command line regardless of which binary name comes first), so using the wrapper is never a way to silently skip scope/budget/audit enforcement. `tool_resolver.py`'s own `_RATE_LIMIT_PATTERNS` widened in the same pass (`retry.?after`, `try again (later|in \d+)`, `slow down`) after a real engagement's own rate-limiter signaled via one of these instead of a literal `429`/`rate limit` string. Live-verified both body-handling paths against local 429-then-200 test servers: stdout mode returns exactly the successful retry's body with no duplication, `-o` file mode ends with the file containing exactly the successful retry's content and none of the failed attempt's error body | Correctly self-corrected mid-plan after a real engagement's own review pointed out the hook-side design was architecturally impossible for a PreToolUse hook to implement; `tests/test_scope_gate_hook.py`/`tests/test_tool_resolver.py` extended accordingly |
| **Extension (2026-08-30) — `browser-mcp`: wire audit logging into the same direct-Playwright calls that already had budget** | ✅ Done. `browser-mcp`'s `browser_confirm.py` and `playwright-mcp`'s `challenge_solver.py` are both direct-Playwright MCP tools (not subprocess calls through `tool_resolver.run_tool()`'s shared budget+audit chokepoint), so each has always had to wire budget/audit itself. `challenge_solver.py` (built after `browser_confirm.py`) got both from the start; `browser_confirm.py` only ever got budget — its own module docstring said so explicitly ("budget IS enforced here directly... " with no audit mention) — so every `check_js_execution`/`render_dom`/`extract_page_content`/`fill_and_submit`/`screenshot_base64` call across every engagement to date was invisible to `data/audit.jsonl`, the one place a real engagement's tool-call history gets reviewed after the fact. Fixed by mirroring `challenge_solver.py`'s exact pattern in all 5 functions: `time.monotonic()` captured right after `_enforce_budget("browser-mcp")`, `audit_log.log_call()` called just before each `return` with the elapsed `duration_ms`, `returncode=None` (no subprocess, nothing to report), and `block=None` (this module has no rate-limit/WAF classification signal to make — that's `playwright-mcp`'s job). `fill_and_submit` deliberately logs `[url, submit_selector]` only, not `field_values` — a form submission being tested can legitimately carry a password or other secret, which has no business sitting in a plaintext audit log. `playwright-mcp/challenge_solver.py`'s own docstring, which explicitly called out this asymmetry ("unlike browser-mcp, which only enforces budget"), updated to point at the fix instead of the now-stale gap. Pure additive logging change — `tests/test_browser_mcp.py`'s existing 6 cases (all against the Playwright-free `_normalize_links()` helper) and the full suite still pass unchanged, confirming the import/module structure wasn't disturbed | Left-over gap from the `playwright-mcp` build (2026-08-29): that work correctly gave the new server both budget and audit, but only flagged — never fixed — the same asymmetry already sitting in `browser-mcp`, the module it was directly modeled on |
...
# Obscura (optional) -- Rust headless browser, ships its own MCP server.
# scripts/connect-obscura.sh registers it as a personal --scope local
# MCP config, same reasoning as Burp above (not tracked config -- its
# calls bypass tool_resolver.py's shared budget/audit logic the way a
# wrapped Python MCP server's wouldn't). See
