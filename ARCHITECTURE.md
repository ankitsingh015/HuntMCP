...
| Scope enforcement on OpenCode (raw Bash) | ✅ Done — `.opencode/plugin/scope-gate.ts`, auto-discovered by OpenCode (any `.ts`/`.js` under `.opencode/plugin/` loads with no `opencode.jsonc` registration needed) and wired to the `tool.execute.before` hook — the documented, currently-shipping OpenCode equivalent of Claude Code's `PreToolUse` (confirmed against `@opencode-ai/plugin`'s shipped type definitions and the official plugin docs' own block-by-throwing example). Before this, OpenCode had **zero** scope/budget/audit enforcement for any raw Bash Tier-2 command — `opencode.jsonc`'s `permission.bash` is a static ask/allow glob-pattern block, it cannot inspect a command's actual target host. Rather than reimplementing scope/budget/audit logic in TypeScript (a second copy that could drift from the Python original), the plugin shells out to the exact same `scripts/hooks/scope_gate_hook.py` over the exact same stdin JSON contract Claude Code already uses — one source of truth, and it inherits the budget/audit wiring above (and any future `scope_guard.py` fix) automatically. The Bash tool's exact identifier (`"bash"`) and argument key (`"command"`) were confirmed directly from the shipped `opencode` binary's own Zod tool schema (`p.Struct({command: p.String...})`) rather than assumed from docs, and end-to-end verified via `tests/test_scope_gate_plugin.mjs` (shims `Bun.spawn` with `node:child_process` so it runs under plain Node without requiring a standalone `bun` binary, exercising the real plugin code against the real hook script). **Deliberately narrow, matching the hook's own scope**: only the `bash` tool is covered. MCP-server-provided tools (`subfinder-mcp`, `httpx-mcp`, `nuclei-mcp`, etc., called as native OpenCode tools rather than raw Bash) are **not** covered — Claude Code's hook already handles this via its `mcp__` branch, so this is a real, pre-existing, and broader Claude-Code/OpenCode asymmetry than what prompted this fix, left as open follow-up rather than guessed at: it needs its own empirical confirmation of how OpenCode names MCP-provided tools in a `tool.execute.before` call before it can be built correctly | Closes the single biggest cross-harness parity gap found this session — scope enforcement existed on only one of the two harnesses despite both running the identical agent roster against the identical live targets |
