...
| Continuous bounty-scope discovery — the 24/7-scanning resource question | 🔶 Diff detection built, unattended active scanning deliberately NOT wired up. Answers a direct design question: scanning ALL ~40k domains across ~850 aggregated programs continuously is neither compute-feasible for a solo operator nor safe/considerate (many programs restrict automated scanning, and blasting every listed program at once looks like abuse regardless of intent). The right split is two tiers, not one: **Tier 1 (cheap, safe to run often)** — `refresh_bounty_scope()` + `list_new_bounty_scope()` just diff a handful of small JSON downloads against the previous snapshot; near-zero compute, no target ever touched, safe on any cadence (hourly is generous given the 30-min upstream sync). **Tier 2 (expensive, stays deliberately opt-in)** — actual recon/scanning of newly-discovered scope is a human decision per target, triggered manually (or via a future budget-capped auto-queue), never blind/unattended across hundreds of live programs at once. This directly matches the instinct "jo naya target add hua usko hunt karenge" — refined to diff-based (only the delta, not a repeated full re-scan) rather than re-scanning the whole universe daily. Wiring Tier 1 into an actual unattended cron job (vs. calling it manually/on-demand) is a separate decision with real operational weight (unattended, automated contact with potentially hundreds of live external programs) — deliberately left for explicit confirmation rather than silently automated | `watch-mcp` already establishes the same tiered instinct for a single already-in-scope target (cheap diffing, expensive action stays deliberate) — this extends the same philosophy to the pre-engagement discovery stage |
