...
| Methodology as Claude Code Skills | 🔶 In progress, deliberately paced — `.claude/skills/writing-great-skills/SKILL.md` (meta-skill, conventions) plus 37 content skills covering 50 of 59 phases so far (Phases 0 through 31 inclusive, all sub-phases). `master-pentest-prompt.md` stays the source of truth for OpenCode (unchanged, still grep-able) — `.claude/skills/` is an additional Claude-Code-only front door, not a replacement; see its own header for the exact current phase→skill mapping rather than duplicating that list here. Deliberately grouped by testing concern, not 1:1 with phase numbers. Content-preservation checked per batch by spot-verifying specific technique strings (CVE IDs, exact bypass syntax, tool names) survived the conversion. **Location correction** (caught via the user's "research to optimize" instruction, before more phases piled onto the wrong spot): the first two batches were initially written to a bare top-level `skills/` — Claude Code only discovers project skills at `.claude/skills/<name>/SKILL.md`, verified against the real docs; moved and all path references fixed. Two existing skills (`auth-and-session`, `access-control-and-idor`) were enhanced in place with user-supplied research (MFA bypass techniques, real disclosed-H1-report IDOR analysis) rather than filed as new Writeup RAG entries — durable, cross-cutting technique knowledge goes into a skill; high-volume per-writeup/per-target material keeps flowing into the existing Writeup RAG / Memory DB pipeline unchanged. Remaining ~9 phases (32 through 37) not yet converted — continuing in further batches per the meta-skill's own guidance against padding a thin conversion just to call it done | Cleaner than one big grep-able file; skills load progressively instead of requiring agents to grep line ranges |
