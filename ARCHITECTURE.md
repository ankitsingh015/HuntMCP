...
| Phase 2.8 — claude-bug-bounty-derived backlog | 🔶 In progress | Scope-enforcement hook done (`scripts/hooks/scope_gate_hook.py`); HackerOne MCP (read-only), OOB listener, WAF bypass tooling still open |
| Phase 2.9 — Strix-derived backlog | 🔶 In progress | Budget circuit-breaker done (`mcp-servers/budget_guard.py`); duplicate-work check, context compaction strategy, Caido support still open |
...
### Phase 2.8: claude-bug-bounty-derived Backlog (In progress)
